    ...
    def __call__(self, X, Y, dx=False, dy=False, eval_gradient=False):
        ...
        for a in range(n):
            for b in range(m):
                ...
                K[da, b] = dot_plus_c**(
                    self.exponent - 1)*self.exponent*Y[b, :]
                K[a, db] = dot_plus_c**(
                    self.exponent - 1)*self.exponent*X[a, :]
                            ...
                            for Gj, dGj, tj in zip(Gsj, dGsj, tsj):
                                if ti == tj:
                                    ...
                                    if self.kernel == 'dot_product':
                                        ...
                                        raise NotImplementedError
                                    kernel_mat[
                                        n+i*n_dim:n+(i+1)*n_dim, j] += (
                                            K.dot(dGi.reshape((-1, n_dim))))
                                    kernel_mat[
                                        i, m+j*n_dim:m+(j+1)*n_dim] += (
                                            K_prime.dot(
                                                dGj.reshape((-1, n_dim))))
                                    kernel_mat[
                                        n+i*n_dim:n+(i+1)*n_dim,
                    ...
                    for j, (Gsj, dGsj, tsj) in enumerate(zip(
                        ...
                        for Gi, dGi, ti in zip(Gsi, dGsi, tsi):
                            ...
                            ddi[:, self.descriptor_pos[ti][0]:
                                self.descriptor_pos[ti][1]] += dGi.reshape(
                                    (-1, 3*len(Gsi))).T
                        ...
                        for Gj, dGj, tj in zip(Gsj, dGsj, tsj):
                            ...
                            ddj[:, self.descriptor_pos[tj][0]:
                                self.descriptor_pos[tj][1]] += dGj.reshape(
                                    (-1, 3*len(Gsj))).T
                        kernel_mat[i, j] = (self.factor*di.dot(dj) +
                                            num_atoms_i.dot(num_atoms_j))

                        kernel_mat[n+i*n_dim:n+(i+1)*n_dim, j] = ddi.dot(
                            dj)*self.factor
                        kernel_mat[i, m+j*n_dim:m+(j+1)*n_dim] = di.dot(
                            ddj.T)*self.factor
                        kernel_mat[n+i*n_dim:n+(i+1)*n_dim,
                                   m+j*n_dim:m+(j+1)*n_dim] = ddi.dot(
                                        ddj.T)*self.factor
                return kernel_mat
            else:
                raise NotImplementedError
        ...
        if not eval_gradient:
            ...
            for ii in range(dx_max + 1):
                for jj in range(dy_max + 1):
                    ...
                    kernel_mat[n * ii:n * (ii + 1),
                               m * jj:m * (jj + 1)] = k_mat
                    kernel_derivative[n * ii:n * (ii + 1),
                                      m * jj:m * (jj + 1), :] = deriv_mat
            ...
            for ii in range(dx_max + 1):
                for jj in range(dy_max + 1):
                    ...
                    #     x1, x2, dx=ii+1, dy=jj+1, eval_gradient=False)
                        ...
                        kernel_mat[(n+ii-1)::dx_max,
                                   (m+jj-1)::dy_max] = kernel(
                            x1, x2, dx=ii, dy=jj, eval_gradient=False)
            return kernel_mat
        else:
            num_theta = len(kernel.theta)
            ...
            for ii in range(dx_max + 1):
                for jj in range(dy_max + 1):
                    ...
                    if ii == 0 and jj == 0:
                        ...
                        kernel_derivative[(n+ii-1)::dx_max,
                                          (m+jj-1)::dy_max] = deriv_mat
        return kernel_mat, kernel_derivative
