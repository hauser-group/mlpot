        ...
        if eval_gradient:
            ...
            if dx != 0 or dy != 0:
                ...
                return (K1*K2_0 + K2 * K1_0,
                        np.dstack((K1_gradient * K2_0[:, :, np.newaxis] +
                                   K1_gradient_0 * K2[:, :, np.newaxis],
                                   K2_gradient * K1_0[:, :, np.newaxis] +
                                   K2_gradient_0 * K1[:, :, np.newaxis])))
...
class ConstantKernel(sk_ConstantKernel):
    def __call__(self, X, Y=None, dx=0, dy=0, eval_gradient=False):
        ...
        if Y is None:
            ...
            if not np.all(X == Y):
                raise ValueError(
                    "Gradient can only be evaluated when Y is None or X is Y.")
        ...
        if dx == 0 and dy == 0:
            ...
            K = np.zeros((X.shape[0], Y.shape[0]),
                         dtype=np.array(self.constant_value).dtype)
        if eval_gradient:
            if (not self.hyperparameter_constant_value.fixed and
                    dx == 0 and dy == 0):
        ...
        if not self.anisotropic or length_scale.shape[0] == 1:
            if dx != 0 or dy != 0:
                if dx == dy:
                    ...
                    K = -K * np.subtract.outer(
                        X[:, dx-1].T, Y[:, dx-1])/length_scale**2  # K_prime
                else:
                    K = -K * np.subtract.outer(
                        X[:, dx-1].T, Y[:, dx-1])*np.subtract.outer(
                        X[:, dy-1].T, Y[:, dy-1])/length_scale**4  # J_ij
        else:
            if dx != 0 or dy != 0:
                if dx == dy:
                    K = K * (1 - np.subtract.outer(
                        X[:, dx-1].T, Y[:, dx-1])**2/length_scale[dx-1]**2
                        )/length_scale[dx-1]**2
                elif dx == 0:
                    K = K * np.subtract.outer(
        ...
        if eval_gradient:
            if self.hyperparameter_length_scale.fixed:
                ...
                if dx == 0 and dy == 0:
                    ...
                    K_gradient = (
                        K * (dists - 2))[:, :, np.newaxis]/length_scale
                elif dy != 0 and dx == 0:
                    # K_gradient = (K * (dists - 2))[:, :, np.newaxis]
                    K_gradient = (
                        K * (dists - 2))[:, :, np.newaxis]/length_scale
                    ...
                    if dx == dy:
                        ...
                        K_gradient = (K * (dists - 4) +
                                      2 * np.exp(-0.5*dists)/length_scale**2
                        ...
                        K_gradient = (
                            K * (dists - 4))[:, :, np.newaxis]/length_scale
                        # K_gradient = (
                ...
                grad = (X[:, np.newaxis, :] - Y[np.newaxis, :, :]) ** 2 / (
                    length_scale ** 2)
                if dx == 0 and dy == 0:
