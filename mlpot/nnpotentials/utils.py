...
def calculate_eam_maps(num_atom_types, _Gs, _types):
    ...
    for i in range(num_atom_types):
        ...
        maps.append(_tf.SparseTensorValue(
            indices[i], [1.0]*Ns[i], [batchsize, Ns[i]]))
        for j in range(num_atom_types):
            b_indices[i][j] = _np.array(
                b_indices[i][j], dtype=_np.int64).reshape((-1, 2))
            b_maps[i][j] = _tf.SparseTensorValue(
                b_indices[i][j], [1.0]*len(r[i][j]), [Ns[i], len(r[i][j])])
...
def calculate_bp_maps(num_atom_types, _Gs, _types):
    ...
    for a in range(num_atom_types):
        ...
        maps.append(_tf.SparseTensorValue(
            indices[a], [1.0]*Ns[a], [batchsize, Ns[a]]))
...
def calculate_bp_indices(num_atom_types, Gs, types, dGs=None):
    ...
    if dGs is None:
        ...
        for a in range(num_atom_types):
            indices[a] = _np.array(
                indices[a], dtype=_np.int32).reshape((-1, 1))
            atoms[a] = _np.array(atoms[a])
