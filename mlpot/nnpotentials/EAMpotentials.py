...
class EAMpotential():
    def __init__(self, atom_types, error_scaling=1000):
        ...
        for t in self.atom_types:
            self.atom_maps[t] = _tf.sparse_placeholder(
                shape=(None, None), dtype=precision, name="%s_map" % t)

    def _post_setup(self):
        self.E_predict = _tf.reduce_sum([
            _tf.sparse_tensor_dense_matmul(
                self.atom_maps[t],
        ...
        with _tf.name_scope("RMSE"):
            self.rmse_weights = _tf.placeholder(
                shape=(None,), dtype=precision, name="weights")
    ...
    def export(self, sess, file_name, atomic_prop_dict, Nrho=10000, drho=0.03,
               Nr=10000, dr=None, cutoff=8.2):
        if dr is None:
            dr = cutoff/Nr
            ...
            for t in self.atom_types:
                ...
                for t2 in self.atom_types:
                    ...
                    for i in range(0, len(rho_tab), 5):
                        # There should be a better way instead of list()
                        fout.write("{:.14E}  {:.14E}  {:.14E}  {:.14E}  "
                                   "{:.14E}\n".format(*list(rho_tab[i:(i+5)])))
...
class EAMAtomicNN():
    def __init__(self, atom_types, offset=0.0, name="ANN"):
        ...
        self.b_maps = {}
        for t in atom_types:
            self.inputs[t] = _tf.placeholder(
                shape=(None, 1), dtype=precision, name="ANN_input_%s" % t)
    ...
    def _post_setup(self):
        ...
        with _tf.variable_scope(
                self.name+"_EmbeddingFunc", reuse=_tf.AUTO_REUSE):
            self.F_out = self.F(self.sum_rho)
        self.output = _tf.add(_tf.reduce_sum(
                [_tf.sparse_tensor_dense_matmul(self.b_maps[t],
                                                self.pairPot[t])
...
class SMATBpotential(EAMpotential):
    def __init__(self, atom_types, initial_params={}, offsets=None,
                 pair_trainable=True, rho_trainable=True, r0_trainable=False):
    ...
    @staticmethod
    def build_pairPot(t12, initial_params, r0_trainable, pair_trainable):
        """ builds the pair potential for a given atom type tuple
            t12 = (t1, t2)
            returns: parameters A, p, r0 and the corresponding pair potential
                     as a function of r
        """
        with _tf.variable_scope("{}_{}_PairPot".format(*t12),
                                reuse=_tf.AUTO_REUSE):
            A_init = initial_params.get(("A", t12[0], t12[1]), 0.2)
            A = _tf.get_variable(
                "A_{}_{}".format(*t12), dtype=precision,
            ...
            p_init = initial_params.get(("p", t12[0], t12[1]), 9.2)
            p = _tf.get_variable(
                "p_{}_{}".format(*t12), dtype=precision,
        ...
        with _tf.variable_scope("{}_{}_r0".format(*t12), reuse=_tf.AUTO_REUSE):
            r0_init = initial_params.get(("r0", t12[0], t12[1]), 2.7)
            r0 = _tf.get_variable(
                "r0_{}_{}".format(*t12), dtype=precision,
    ...
    @staticmethod
    def build_rho(t12, initial_params, r0_trainable, rho_trainable):
        """ builds the rho contribution for a given atom type tuple
            t12 = (t1, t2)
            returns: parameters xi, q, r0 and the corresponding rho
                     contribution as a function of r
        """
        with _tf.variable_scope("{}_{}_rho".format(*t12),
                                reuse=_tf.AUTO_REUSE):
            xi_init = initial_params.get(("xi", t12[0], t12[1]), 1.6)
            xi = _tf.get_variable(
                "xi_{}_{}".format(*t12), dtype=precision,
                initializer=_tf.constant(xi_init, dtype=precision),
                trainable=rho_trainable,
                collections=[_tf.GraphKeys.MODEL_VARIABLES,
                             _tf.GraphKeys.GLOBAL_VARIABLES])
            _tf.summary.scalar("xi_{}_{}".format(*t12), xi,
                               family="modelParams")

            q_init = initial_params.get(("q", t12[0], t12[1]), 3.5)

            q = _tf.get_variable(
                "q_{}_{}".format(*t12), dtype=precision,
...
class NNEpotential(EAMpotential):
    def __init__(self, atom_types, layers=[20], initial_params=None, cut_a=5.4,
                 cut_b=8.1, r0_trainable=False, pair_trainable=True,
                 rho_trainable=True, offsets=None):
        with _tf.variable_scope("NNEpot"):
            ...
            for t1, offset in zip(self.atom_types, offsets):
                with _tf.variable_scope("{}_ANN".format(t1),
                                        reuse=_tf.AUTO_REUSE):
                    self.ANNs[t1] = EAMAtomicNN(atom_types, offset)
    ...
    def build_F(t1, initial_params, layers):
        with _tf.variable_scope("%s_EmbeddingFunc" % t1, reuse=_tf.AUTO_REUSE):
            ...
            mu = _tf.get_variable(
                "mu_rho_{}".format(t1), dtype=precision,
                initializer=_tf.constant(mu_init, dtype=precision),
                trainable=False,
                collections=[_tf.GraphKeys.MODEL_VARIABLES,
                             _tf.GraphKeys.GLOBAL_VARIABLES])
            std = _tf.get_variable(
                "std_rho_{}".format(t1), dtype=precision,
            ...
            for i, n in enumerate(layers):
                if i == 0:
                    layer, weights, bias = nn_layer(
                        (rho - mu)/std, 1, n, name="hiddenLayer_%d" % (i+1))
                else:
                    # Use previous layer as input
                    layer, weights, bias = nn_layer(
                        layer, layers[i-1], n, name="hiddenLayer_%d" % (i+1))
            output, _, _ = nn_layer(
                layer, layers[-1], 1, act=None, name="outputLayer")
            return -_tf.sqrt(rho)*output
        return F


class NNRHOpotential(EAMpotential):
    def __init__(self, atom_types, layers=[20], initial_params=None,
                 r0_trainable=False, pair_trainable=True, cut_a=5.4,
                 cut_b=8.1, offsets=None):
        with _tf.variable_scope("NNRHOpot", reuse=_tf.AUTO_REUSE):
            ...
            for t1, t2 in combinations_with_replacement(atom_types, r=2):
                ...
                rho[t12] = NNRHOpotential.build_rho(
                    t12, initial_params, layers, cut_a, cut_b, r0_trainable)
    ...
    def build_rho(t12, initial_params, layers, cut_a, cut_b, r0_trainable):
        ...
        def rho(r):
            for i, n in enumerate(layers):
                if i == 0:
                    layer, weights, bias = nn_layer(
                        r/r0-1, 1, n, name="hiddenLayer_%d" % (i+1))
                else:
                    # Use previous layer as input
                    layer, weights, bias = nn_layer(
                        layer, layers[i-1], n, name="hiddenLayer_%d" % (i+1))
...
class NNVRHOpotential(EAMpotential):
    def __init__(self, atom_types, layers_V=[20], layers_rho=[20],
        ...
        with _tf.variable_scope("NNVRHOpot", reuse=_tf.AUTO_REUSE):
            ...
            for t1, t2 in combinations_with_replacement(atom_types, r=2):
                t12 = tuple(sorted([t1, t2]))
                pairPot[t12] = NNVRHOpotential.build_pairPot(
                    t12, initial_params, layers_V, cut_a, cut_b, r0_trainable)

                rho[t12] = NNRHOpotential.build_rho(
                    t12, initial_params, layers_rho, cut_a, cut_b,
                    r0_trainable)
...
class NNVERHOpotential(EAMpotential):
    def __init__(self, atom_types, layers_F=[20], layers_V=[20],
                 layers_rho=[20], initial_params=None, r0_trainable=False,
                 cut_a=5.4, cut_b=8.1, offsets=None):
        with _tf.variable_scope("NNVERHOpot", reuse=_tf.AUTO_REUSE):
            ...
            for t1, t2 in combinations_with_replacement(atom_types, r=2):
                t12 = tuple(sorted([t1, t2]))
                pairPot[t12] = NNVERHOpotential.build_pairPot(
                    t12, initial_params, layers_V, cut_a, cut_b, r0_trainable)
    ...
    def build_pairPot(t12, initial_params, layers, cut_a, cut_b, r0_trainable):
        ...
        def V(r):
            ...
            output, _, _ = nn_layer(
                layer, layers[-1], 1, act=None, name="outputLayer")
            return A*((1.0 - _tf.exp(output))**2 - 1) * poly_cutoff(
                r, cut_a, cut_b)
        return V
    ...
    def __init__(self, atom_types, layers_F=[20], layers_rho=[20],
        ...
        with _tf.variable_scope("NNfreeERHOpot", reuse=_tf.AUTO_REUSE):
            ...
            for t1, t2 in combinations_with_replacement(atom_types, r=2):
                ...
                rho[t12] = NNfreeERHOpotential.build_rho(
                    t12, initial_params, layers_rho, cut_a, cut_b,
                    r0_trainable)
