    ...
    def test_gold_dataset(self):
            ...
            try:
                ...
                (Gs_train, types_train, E_train,
                 Gs_test, types_test, E_test) = pickle.load(
                    fin, encoding='latin1')
        pot = build_BPestimator(
            ["Au"], [len(Gs_train[0][0])], layers=[[64, 64]])
            ...
            def gen():
                for i in range(0, len(Gs_train), batch_size):
                    [Au_atoms], [Au_indices] = calculate_bp_indices(
                        1, Gs_train[i:(i+batch_size)],
                        types_train[i:(i+batch_size)])
                    yield({'Au_input': Au_atoms, 'Au_indices': Au_indices},
                          E_train[i:(i+batch_size)])
            train_data = tf.data.Dataset.from_generator(
                gen,
                ({'Au_input': tf.float32, 'Au_indices': tf.int32}, tf.float32),
                ({'Au_input': tf.TensorShape([None, len(Gs_train[0][0])]),
                 'Au_indices': tf.TensorShape([None, 1])},
                 tf.TensorShape([None, ])))
        ...
        def test_input_fn():
            test_data = tf.data.Dataset.from_tensor_slices(
                ({'Au_input': np.expand_dims(Au_atoms, axis=0).astype(
                    np.float32),
